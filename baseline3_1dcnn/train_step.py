...
from module.trainer_step import Trainer
...
def get_config():
    ...
    parser.add_argument("--global_attn", default=0, type=int)
...
def get_data_files(args):
    ...
    return all_texts, token_weights, data, csv, all_train_ids, val_ids, val_files
...
def get_model(args, train_dataloader):
    ...
    lr_schedule = np.r_[np.linspace(args.lr, args.lr, args.warmup_steps),
                    (np.cos(np.linspace(0, np.pi, len(train_dataloader) - args.warmup_steps)) * .5 + .5) * (args.lr - args.min_lr)
                    + args.min_lr]
...
if __name__ == "__main__":
    ...
    trainer = Trainer(model, train_dataloader, val_dataloader, lr_schedule, opt, label_names, args)
