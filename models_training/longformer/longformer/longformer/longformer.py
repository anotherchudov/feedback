...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if extra_attention_mask is not None:
            ...
            selected_attn_weights[selection_padding_mask_zeros[0], :, :, selection_padding_mask_zeros[1]] = torch.tensor(-10000.,                                                                                                                         device=selected_attn_weights.device).expand(len(selection_padding_mask_zeros[0]),
        ...
        if extra_attention_mask is not None:
            ...
            attn_weights[selection_padding_mask_zeros[0], :, selection_padding_mask_zeros[1], :] = torch.tensor(-10000., 
                                                                                                device=attn_weights.device).expand((len(selection_padding_mask_zeros[0]), 
