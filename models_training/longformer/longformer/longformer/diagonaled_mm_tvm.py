...
class DiagonaledMM(torch.autograd.Function):
    ...
    def _compile_function(dtype: str, device: str, b0: int = 4, b1: int = 4, b2: int = 16):
        ...
        D = te.placeholder((h), name='D', dtype='int')  # dilation per head
        output_shape = (b, n, h, t3d3)  # shape of the result tensor
        algorithm = lambda l, i, q, j: te.sum(
            te.if_then_else(
        ...
        Z = te.compute(output_shape, algorithm, name='Z')  # automatically generate cuda code
