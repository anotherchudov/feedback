...
run = wandb.init(entity='schudov', project='feedback_longformer_large')
run.name = f'fold{val_fold}_minlr{min_lr}_maxlr{lr}_wd{wd}_warmup{warmup_steps}_gradnorm{max_grad_norm}_biasdecay{decay_bias}_ls{label_smoothing}_wp{weights_pow}_data{dataset_version}_attn{global_attn}'
...
class TrainDataset(t.utils.data.Dataset):
    def __init__(self, ids):
        self.ids = ids
        if data_prefix != '':
            self.data = h5py.File(f'../../conformal_data_v{dataset_version}.h5py')
        else:
            self.data = h5py.File(f'../../processed_data_v{dataset_version}.h5py')
    def __len__(self):
        return len(self.ids)
...
class ValDataset(t.utils.data.Dataset):
    def __init__(self, ids):
        self.ids = ids
        self.data = h5py.File('../../processed_data_v0.h5py')
...
class TvmLongformer(t.nn.Module):
    def __init__(self):
        super().__init__()
        config = LongformerConfig.from_pretrained('pretrained_checkpoints/longformer-large-4096/') 
        config.attention_mode = 'sliding_chunks'
        self.feats = Longformer.from_pretrained('pretrained_checkpoints/longformer-large-4096/', config=config)
    ...
    def forward(self, tokens, mask):
        return t.log_softmax(self.class_projector(self.feats(tokens, mask, return_dict=False)[0]), -1)
    ...
    if step_ % grad_acc_steps == grad_acc_steps - 1:
        ...
        if step % eval_interval == (eval_interval - 1):
            ...
            if len(past_averaged_params) > 25:
                past_averaged_params = past_averaged_params[-25:]
