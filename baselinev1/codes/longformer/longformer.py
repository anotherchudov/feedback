...
from ducky_transformers import RobertaConfig, RobertaModel, RobertaForMaskedLM
...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if extra_attention_mask is not None:
            ...
            # print(selected_attn_weights.shape)
            # print((len(selection_padding_mask_zeros[0]), *selected_attn_weights.shape[1:3], len(selection_padding_mask_zeros[1])),
            #           selected_attn_weights[selection_padding_mask_zeros[0], :, :, selection_padding_mask_zeros[1]].shape)
            selected_attn_weights[selection_padding_mask_zeros[0], :, :, selection_padding_mask_zeros[1]] = torch.tensor(-10000.,                                                                                                                         device=selected_attn_weights.device, dtype=selected_attn_weights.dtype).expand(len(selection_padding_mask_zeros[0]),
                                                                                                                        *selected_attn_weights.shape[1:3])
            
            #torch.full((len(selection_padding_mask_zeros[0]), *selected_attn_weights.shape[1:3], len(selection_padding_mask_zeros[1])), -10000, device=selected_attn_weights.device)
        ...
        if extra_attention_mask is not None:
            ...
            attn_weights[selection_padding_mask_zeros[0], :, selection_padding_mask_zeros[1], :] = torch.tensor(-10000., 
                                                                                                device=attn_weights.device, dtype=attn_weights.dtype).expand((len(selection_padding_mask_zeros[0]), 
            ...
            reshaped_gathered_attn = nonzero_selected_attn.reshape(len(selection_padding_mask_nonzeros[0]), -1).type_as(hidden_states)
            attn[extra_attention_mask_nonzeros[1],
                 extra_attention_mask_nonzeros[0], :] = reshaped_gathered_attn
